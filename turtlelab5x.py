...
class Task:
    ...
    def start(self):
        stage.reset()
        self.mickey = Home(randint(50,300),randint(50,300),32,32,IMG_HOME,"Mickey")
        self.raph = Home(-randint(50,300),randint(50,300),32,32,IMG_HOME,"Raph")
        self.leo = Home(-randint(50,300),-randint(50,300),32,32,IMG_HOME,"Leo")
        self.donnie = Home(randint(50,300),-randint(50,300),32,32,IMG_HOME,"Donnie")
        stage.add_object(self.mickey)
        stage.add_object(self.raph)
        stage.add_object(self.leo)
        stage.add_object(self.donnie)
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Mickey's home is at ({self.mickey.x},{self.mickey.y})")
            print(f"Raph's home is at ({self.raph.x},{self.raph.y})")
            print(f"Leo's home is at ({self.leo.x},{self.leo.y})")
            print(f"Donnie's home is at ({self.donnie.x},{self.donnie.y})")

    def done(self):

        def visited(friend):
            if any([friend.contains(stop) for stop in stage.stops]):
                return "PASSED"
            else:
                return "FAILED"
        ...
        if INTERACTIVE:
            print("Evaluating...")
        print("Check if Turtle visited Mickey: ", end="")
        print(visited(self.mickey))
        print("Check if Turtle visited Raph: ", end="")
        print(visited(self.raph))
        print("Check if Turtle visited Leo: ", end="")
        print(visited(self.leo))
        print("Check if Turtle visited Donnie: ", end="")
        print(visited(self.donnie))
...
mod.check = task.done
mod.turtle = stage.turtle
mod.mickey = task.mickey
mod.raph = task.raph
mod.leo = task.leo
mod.donnie = task.donnie
