...
class Task:
    """
    TASK: help guide Turtle back home while stop to visit Crush and
    Koopa Crush's, Koopa's, and Turtle's home will randomly appear
    ANYWHERE on the map.
    """
    def start(self):
        stage.reset()
        ...
        if INTERACTIVE:
            ...
            shuffle(locations)
            (crush_x,crush_y),(koopa_x,koopa_y) = locations
        else:
            crush_x,crush_y = int(input()),int(input())
            koopa_x,koopa_y = int(input()),int(input())
            home_x,home_y = int(input()),int(input())

        self.home = Home(home_x,home_y,32,32,IMG_HOME,"home")
        self.crush = Home(crush_x,crush_y,32,32,IMG_HOME,"crush")
        self.koopa = Home(koopa_x,koopa_y,32,32,IMG_HOME,"koopa")
        stage.add_object(self.home)
        stage.add_object(self.crush)
        stage.add_object(self.koopa)
        stage.add_object(Road(0,0,crush_x,crush_y,5,"yellow"))
        stage.add_object(Road(0,0,koopa_x,koopa_y,5,"yellow"))
        stage.add_object(Road(crush_x,crush_y,koopa_x,koopa_y,5,"yellow"))
        stage.add_object(Road(home_x,home_y,crush_x,crush_y,5,"yellow"))
        stage.add_object(Road(home_x,home_y,koopa_x,koopa_y,5,"yellow"))
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Crush appears at ({self.crush.x:.2f},{self.crush.y:.2f})")
            print(f"Koopa appears at ({self.koopa.x:.2f},{self.koopa.y:.2f})")
            print(f"Home appears at ({self.home.x:.2f},{self.home.y:.2f})")
        stage.total_distance = 0
        return stage.turtle,self.home,self.crush,self.koopa

    def shortest_path(self):
        dist1 = ( distance(0,0,self.crush.x,self.crush.y) +
                  distance(self.crush.x,self.crush.y,self.koopa.x,self.koopa.y) +
                  distance(self.koopa.x,self.koopa.y,self.home.x,self.home.y) )
        dist2 = ( distance(0,0,self.koopa.x,self.koopa.y) +
                  distance(self.koopa.x,self.koopa.y,self.crush.x,self.crush.y) +
                  distance(self.crush.x,self.crush.y,self.home.x,self.home.y) )
    ...
    def done(self):
        ...
        if INTERACTIVE:
            ...
            print("Seed =",6)
        print("Check if Turtle stopped at Crush's: ", end="")
        if any(self.crush.contains(stop) for stop in stage.stops):
            print("PASSED")
        else:
            print("FAILED")
        print("Check if Turtle stopped at Koopa's: ", end="")
        if any(self.koopa.contains(stop) for stop in stage.stops):
            print("PASSED")
        else:
            print("FAILED")
...
task = Task()
turtle,home,crush,koopa = task.start()
mod.check = task.done
mod.turtle = turtle
mod.home = home
mod.crush = crush
