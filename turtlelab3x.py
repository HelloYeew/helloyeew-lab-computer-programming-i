...
class Task:
    ...
    def start(self):
        ...
        shop_x = randint(50,150)
        shop_y = randint(50,150)
        home_x = randint(shop_x+50,shop_x+150)
        home_y = randint(shop_y+50,shop_y+150)
        self.home = Home(home_x,home_y,32,32,IMG_HOME,"home")
        self.shop = Home(shop_x,shop_y,32,32,IMG_SHOP,"shop")
        stage.add_object(self.home)
        stage.add_object(self.shop)
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Shop appears at ({self.shop.x},{self.shop.y})")
            print(f"Home appears at ({self.home.x},{self.home.y})")
        stage.total_distance = 0
        return stage.turtle,self.home,self.shop

    def done(self):
        verify_turtle()
        if INTERACTIVE:
            print("Evaluating...")
        print("Check if Turtle stopped at the shop: ", end="")
        if any(self.shop.contains(stop) for stop in stage.stops):
            print("PASSED")
        else:
            print("FAILED")
        ...
        if self.home.contains(stage.turtle):
            ...
            shortest = ( sqrt(self.shop.x**2 + self.shop.y**2) +
                         sqrt((self.home.x-self.shop.x)**2 + (self.home.y-self.shop.y)**2))
            if stage.total_distance < shortest + 5:
                print("PASSED")
            else:
                print("FAILED")
        else:
