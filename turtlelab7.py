...
class Task:
    ...
    TESTS = [
        ...
        {
            "funcs" : "draw_polygon",
            "cmd"   : "turtle.forward(100); draw_polygon(3,100); turtle.left(180); turtle.forward(100);"
                      "turtle.right(90); turtle.forward(100); draw_polygon(4,100); turtle.left(180);"
                      "turtle.forward(100); turtle.right(90); turtle.forward(100); draw_polygon(5,100);"
                      "turtle.left(180); turtle.forward(100); turtle.right(90); turtle.forward(100);"
                      "draw_polygon(6,100); turtle.left(180); turtle.forward(100)",
            "sol"   : {
                "movement": [
                    '(-100.0,0.0)-(0.0,0.0)',
                    '(-100.0,0.0)-(0.0,0.0)',
    ...
    def get_movement(self):
        ...
        movement = []
        prev = stage.stops[0]
        for stop in stage.stops[1:]:
            segment = [prev,stop]
            segment.sort()
            (x1,y1),(x2,y2) = segment
            # round and add 0 to prevent negative zero
            x1 = round(x1,-1) + 0
            y1 = round(y1,-1) + 0
            x2 = round(x2,-1) + 0
            y2 = round(y2,-1) + 0
            # add 0 to prevent negative zero
            p1 = f"({x1:.{prec}f},{y1:.{prec}f})"
            p2 = f"({x2:.{prec}f},{y2:.{prec}f})"
            movement.append(f"{p1}-{p2}")
            movement.sort()
        return movement

    def check(self,test,check_sol=True):
        func_list = [f.strip() for f in test["funcs"].split(",")]
        try:
            for func_name in func_list:
                func = inspect.currentframe().f_back.f_back.f_locals[func_name]
                exec(f"{func_name} = func")
        except KeyError:
        ...
        stage.reset()
        turtle = stage.turtle
        cmd_list = [c.strip() for c in test["cmd"].split(";")]
        for cmd in cmd_list:
            print(f"Executing: {cmd}")
            exec(cmd)
        if check_sol:
            sol = test["sol"]
            print("Check movement: ", end="")
            if self.get_movement() == sol["movement"]:
                print("PASSED")
            else:
                print("FAILED")
            print("Check heading: ", end="")
            if turtle.heading == sol["heading"]:
                print("PASSED")
            else:
                print("FAILED")
            print("----------------------")
            ...
            print(f"heading: {turtle.heading:.0f}")
...
# mask everything inside the module except tasks and turtle
