...
class Task:
    """
    TASK: Help Turtle to visit his friend, Mario.
    ...
    def start(self):
        ...
        if INTERACTIVE:
            ...
            angle = randint(0,360)
            mario_x = cos(radians(angle))*distance
            mario_y = sin(radians(angle))*distance
        else:
            mario_x = int(input("Enter Mario's x: "))
            mario_y = int(input("Enter Mario's y: "))
        self.mario = Home(mario_x,mario_y,32,32,IMG_HOME,"Mario")
        stage.add_object(self.mario)
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Mario's home is at ({self.mario.x:.2f},{self.mario.y:.2f})")
    ...
    def done(self):
        ...
        if INTERACTIVE:
            ...
            print("Seed =",9)
        print("Check if Turtle visited Mario: ", end="")
        print(visited(self.mario))
